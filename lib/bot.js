...
const { isThreadReply, getDirectAnswer, THREAD_NUDGE, THANK_YOU } = require('./dm-flow');
...
function createApp(receiver) {
  ...
  async function handleDmReply({ message, say, client }) {
...
async function openDmAndAsk(app, userId) {
  if (await store.getSession(userId)) {
